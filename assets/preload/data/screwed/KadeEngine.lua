...
function onCreatePost()
    ...
    makeLuaText("scoreTxtKade", "Score:0 | Misses:0 | Accuracy:0% | FC", 0, getProperty('scoreTxt.x') + 362.5, getProperty('scoreTxt.y') + 10)
    setTextSize('scoreTxtKade', 16)
    setTextFont("scoreTxtKade", "kade.ttf")
    setTextAlignment('scoreTxtKade', 'CENTER')
    setObjectOrder('scoreTxtKade', 40)

    makeLuaText("infoTxtKade", songNameNoSpace .. " " .. difficultyNameUpper .. " - Dave Engine", 0, 10, 690)
...
function onGameOver()
    songended = true
    setPropertyFromClass('lime.app.Application', 'current.window.title', "Friday Night Funkin': You suck !")
...
function onUpdate()
    if not inGameOver then
        ...
        if botPlay then
            setTextString("scoreTxtKade", "BOTPLAY")
        else
            if hits > 0 then
                setTextString("scoreTxtKade", "Score:" .. score .. " | Misses:" .. misses .. " | Accuracy:" .. math.floor(acc * 100) / 100 .. "% | (" .. fctext .. ") " .. calculatedtype)
            else
                setTextString("scoreTxtKade", "Score:0 | Misses:0 | Accuracy:0% | FC")
            end
        end
    end
end
...
function onUpdatePost()
    if songended == false then
        ...
        setPropertyFromClass('lime.app.Application', 'current.window.title', "Friday Night Funkin' | VS DAVE")
    end
